...
from numpy import array, loadtxt, sqrt, sum, transpose, zeros
...
def self_consistency(simulation_name, parameters, data_path):
    ...
    clusters_pool = array(init_csd_from_random_null(size=256, ensembles=1), dtype=float)
    ...
    for _ in tqdm(range(num_steps)):
        ...
        cluster = clusters_pool[cluster_index]

        if int(cluster) in residues:
            noise = random_from_residue(residues[int(cluster)])
        else:
            num_overflow += 1
            clusters_pool[cluster_index] = choice(samples)
            continue
        ...
        clusters_pool[cluster_index] = updated_cluster
    ...
    plt.legend()
    plt.savefig(data_path + file_root + "sc2.png")
    plt.show()
