...
def fig2_exp(model_dataset, scanlon, null_dataset, main_fig):
    base_path = f"./results"
    ...
    for row, subfig in enumerate(tqdm(subfigs)):
        ...
        for col, ax in enumerate(axs):
            ...
            if row == 0 and col == 0:
                ax.semilogy(cluster_sizes, changes_icdf, "b-", label="Model")
                ax.semilogy(null_cluster_sizes, null_changes_icdf, "0.7", label="Null model")
            else:
                ax.semilogy(cluster_sizes, changes_icdf, "b-")
                ax.semilogy(null_cluster_sizes, null_changes_icdf, "0.7")
    ...
    if main_fig:
        fig_name = f"./figures/fig2_exp_{null_dataset}.png"
    else:
        fig_name = f"./figures/fig2_exp_{null_dataset}_appendix.png"
